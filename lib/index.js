  ...
  interpolated = interpolated.replace(
    ...
    function (
      ...
      if (escapables.hasOwnProperty(escapee)) {
        ...
        return ( +escapee <= lastCaptureIndex ? matches [escapee] : wholeMatch );
...
var interpolateSlash = function (matches, replacement) {
  var interpolated = replacement;
  var max = matches.length;

  interpolated = interpolated.replace(
    /(\\)([1-9]\d*|[0nrtv\W]|([uUx])(?:\{([0-9a-fA-F]+)\}|([0-9a-fA-F]{2,4})))/g,
    function (
      wholeMatch,
      slash, escapee, codePointPrefix, bracedCodePoint, unbracedCodePoint
    ) {
      var escapables  = { '0' : '\0', n : '\n', r : '\r', t : '\t', v : '\v' };
      var codePoint = unbracedCodePoint || bracedCodePoint;
      var lastCaptureIndex = matches.length - 3;
...
var interpolateDollar = function (matches, replacement) {
  var interpolated = replacement;
  var max = matches.length;
...
module.exports = {
  ...
  interpolateSlash  : interpolateSlash,
  interpolateDollar : interpolateDollar
};
